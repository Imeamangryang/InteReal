...
void AHarnessManager::BeginPlay()
{
    ...
    UHarnessPipelineManager* PipelineManager = GetWorld()->GetSubsystem<UHarnessPipelineManager>();
    ...
    if (PipelineManager && NetworkComponent && SaveManagerComponent && HarnessComponent)
    {
        ...
        // 추가: HUD를 가져와서 파이프라인 바인딩
        if (APlayerController* PC = UGameplayStatics::GetPlayerController(GetWorld(), 0))
        {
            if (AInteRealHUD* HUD = Cast<AInteRealHUD>(PC->GetHUD()))
            {
                HUD->BindHarnessPipeline(PipelineManager);
            }
        }
        
        // 파이프라인 완료 시 후속 작업(그리드, 미니맵) 수행을 위해 바인딩
        PipelineManager->OnPipelineLoadFinished.RemoveAll(this);
        PipelineManager->OnPipelineLoadFinished.AddDynamic(this, &AHarnessManager::DelayedCapture); 
    }
...
{
    UE_LOG(LogTemp, Log, TEXT("[Harness] AHarnessManager::DelayedCapture() 시작"));
