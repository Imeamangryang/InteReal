...
void UHarnessGeneratorComponent::AssembleStructuralWalls(const FHarnessFloorData& FloorData)
{
    ...
    auto BuildOpeningsForRun = [&](const TSet<FString>& EdgeIds, const FVector2D& RunCenter, const FVector2D& RunDirection, float RunLength) -> TArray<FMergedOpening>
    {
        ...
        const FVector2D RunNormal(-RunDirection.Y, RunDirection.X);
        const float HalfRunLength = FMath::Max(RunLength * 0.5f, 0.0f);
        const float SpanMatchLateralTolerance = FMath::Max(HarnessDefaultWallThicknessCm, HarnessMergeEndpointToleranceCm * 2.0f);
        ...
        for (const FTopologyOpening& Opening : FloorData.openings)
        {
            ...
            {
                EffectiveOpeningWidth = FMath::Max(Opening.measured_width_cm, 1.0f);
            }
            else if (Opening.drawn_width_cm > UE_SMALL_NUMBER)
            {
                EffectiveOpeningWidth = FMath::Max(Opening.drawn_width_cm, 1.0f);
            }

            bool bMatchesRun = false;
            if (Opening.bHasSpan)
            {
                ...
                OpeningCenter = (SpanStart + SpanEnd) * 0.5f;

                const float SpanLength = FVector2D::Distance(SpanStart, SpanEnd);
                const float CenterX = FVector2D::DotProduct(OpeningCenter - RunCenter, RunDirection);
                const float LateralDistance = FMath::Abs(FVector2D::DotProduct(OpeningCenter - RunCenter, RunNormal));
                const float SpanHalfExtent = FMath::Max(EffectiveOpeningWidth, SpanLength) * 0.5f;

                bMatchesRun =
                    CenterX >= -HalfRunLength - SpanHalfExtent - HarnessMergeEndpointToleranceCm &&
                    CenterX <= HalfRunLength + SpanHalfExtent + HarnessMergeEndpointToleranceCm &&
                    LateralDistance <= SpanMatchLateralTolerance;
            }

            if (!bMatchesRun && !Opening.target_edge_id.IsEmpty() && EdgeIds.Contains(Opening.target_edge_id))
            {
                if (const FTopologyHalfEdge* const* OpeningEdgePtr = HalfEdgeById.Find(Opening.target_edge_id))
                {
                    FVector2D EdgeStart;
                    FVector2D EdgeEnd;
                    if (GetEdgePoints(**OpeningEdgePtr, EdgeStart, EdgeEnd))
                    {
                        ...
                        if (EdgeLength > KINDA_SMALL_NUMBER)
                        {
                            ...
                            const float Offset = FMath::Clamp(Opening.offset_to_center_cm, 0.0f, EdgeLength);
                            OpeningCenter = Opening.offset_from.Equals(TEXT("end"), ESearchCase::IgnoreCase)
                                ? EdgeEnd - (EdgeDirection * Offset)
                                : EdgeStart + (EdgeDirection * Offset);
                            bMatchesRun = true;
                        }
                    }
                }
    ...
    auto FindFaceSide = [&](const FTopologyHalfEdge& Edge, const FTopologyFace& Face, FWallSurfaceSide& OutSide) -> bool
    {
        ...
        for (int32 i = 0; i < NumVerts; ++i)
        {
            ...
            OutSide.Height = CommonWallHeightCm;
    ...
    auto BuildWallBox = [&](const FVector2D& Center2D, float Angle, float Length, float Depth, float BottomZ, float TopZ, const TArray<FMergedOpening>& Openings, const TArray<FName>& Tags, bool bEditable, const FString& WallEntityId, const FString& WallRole)
    {
        ...
        TArray<FString> MetadataTags = {
            FString::Printf(TEXT("HarnessWallId=%s"), *WallEntityId),
            FString::Printf(TEXT("HarnessWallRole=%s"), *WallRole)
        };
        for (const FMergedOpening& MergedOpening : Openings)
        {
            if (MergedOpening.Opening)
            {
                MetadataTags.Add(FString::Printf(TEXT("HarnessOpeningId=%s"), *MergedOpening.Opening->id));
                MetadataTags.Add(FString::Printf(TEXT("HarnessOpeningType=%s"), *MergedOpening.Opening->type));
                MetadataTags.Add(FString::Printf(TEXT("HarnessOpeningConnectionType=%s"), *MergedOpening.Opening->connection_type));
            }
        }
        AddGeneratedComponentTags(WallComp, TEXT("Wall"), WallEntityId, MetadataTags);

        if (bEditable)
        {
            WallComp->ComponentTags.AddUnique(TEXT("EditableWall"));
        }
        else if (WallRole.Contains(TEXT("WallCore"), ESearchCase::IgnoreCase))
        {
            WallComp->ComponentTags.AddUnique(TEXT("StructuralWallCore"));
            WallComp->ComponentTags.AddUnique(TEXT("MaterialEditIgnore"));
        }
        ...
        WallComp->SetRelativeScale3D(FVector::OneVector);
        ...
        WallComp->SetCastShadow(true);
        WallComp->SetAffectDistanceFieldLighting(true);
        WallComp->SetVisibleInRayTracing(true);
        if (bEditable)
        {
            WallComp->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
            WallComp->SetCollisionObjectType(ECC_WorldStatic);
            WallComp->SetCollisionResponseToAllChannels(ECR_Block);
            WallComp->SetCollisionResponseToChannel(ECC_Pawn, ECR_Block);
            WallComp->SetCollisionResponseToChannel(ECC_Camera, ECR_Block);
            WallComp->SetCollisionResponseToChannel(ECC_GameTraceChannel1, ECR_Block);
            WallComp->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);
        }
        else
        {
            WallComp->SetCollisionProfileName(TEXT("BlockAll"));
            WallComp->SetCollisionResponseToChannel(ECC_GameTraceChannel1, ECR_Ignore);
            WallComp->SetCollisionResponseToChannel(ECC_Visibility, ECR_Ignore);
        }
    ...
    auto BuildWallSurfacePanels = [&](const FWallSurfaceSide& Side, const FVector2D& SurfaceNormal, float WallThickness, const TArray<FName>& Tags, const FString& SurfaceEntityId, const FString& SurfaceRole)
    {
        ...
        // Surface endpoint insets are intentionally ignored for visible finish panels.
        // Z-fighting prevention remains on WallCore via ApplyWallRunEndpointInsets().
        ...
        const float ZMax = Side.ZOffset + FMath::Max(Side.Height, 1.0f);
        if (ZMax - ZMin <= 1.0f)
        {
            return;
        }
        ...
        TArray<FString> SurfaceMetadataTags = {
            FString::Printf(TEXT("HarnessWallId=%s"), *Side.SurfaceEdgeId),
            FString::Printf(TEXT("HarnessSpaceId=%s"), *Side.FaceId),
            FString::Printf(TEXT("HarnessWallRole=%s"), *SurfaceRole)
        };
        for (const FString& CoreEdgeId : Side.CoreEdgeIds)
        {
            SurfaceMetadataTags.Add(FString::Printf(TEXT("HarnessCoreEdgeId=%s"), *CoreEdgeId));
        }
        AddGeneratedComponentTags(SurfaceComp, TEXT("WallSurface"), SurfaceEntityId, SurfaceMetadataTags);
        SurfaceComp->ComponentTags.AddUnique(TEXT("EditableWall"));
        SurfaceComp->ComponentTags.AddUnique(TEXT("MaterialEditableWallSurface"));
        SurfaceComp->ComponentTags.AddUnique(TEXT("Wall"));
        for (const FName& Tag : Tags)
        {
            SurfaceComp->ComponentTags.AddUnique(Tag);
        }

        SurfaceComp->SetRelativeScale3D(FVector::OneVector);
        ...
        SurfaceComp->SetCastShadow(true);
        SurfaceComp->SetAffectDistanceFieldLighting(true);
        SurfaceComp->SetVisibleInRayTracing(true);
    ...
    auto BuildMergedWallRuns = [&]() -> TArray<FWallRun>
    {
        ...
        for (int32 BaseIndex = 0; BaseIndex < SourceEdges.Num(); ++BaseIndex)
        {
            ...
            Run.Openings = BuildOpeningsForRun(Run.OpeningEdgeIds, Run.Center, Run.Direction, Run.Length);
            Runs.Add(Run);
        }
    ...
    auto BuildMergedFaceSides = [&](const FTopologyFace& Face) -> TArray<FWallSurfaceSide>
    {
        ...
        for (int32 i = 0; i < NumVerts; ++i)
        {
            ...
            while (MatchIndex < EdgeMatches.Num())
            {
                ...
                Side.Height = CommonWallHeightCm;
        ...
        {
            Side.SurfaceEdgeId = MakeMergedSurfaceEdgeId(Side.SourceSurfaceEdgeIds);
            Side.Openings = BuildOpeningsForRun(Side.OpeningEdgeIds, Side.Center, Side.Direction, Side.Length);
        }
    ...
    for (const FWallRun& Run : WallRuns)
    {
        ...
        float CoreTopZ = CommonWallHeightCm;
            ...
            for (const FTopologyFace& Face : FloorData.faces)
            {
                ...
                if (FindFaceSide(*Edge, Face, Side))
                {
                    ...
                    CoreTopZ = bHasAdjacentFace
                        ? FMath::Max(CoreTopZ, Face.z_offset + CommonWallHeightCm)
                        : Face.z_offset + CommonWallHeightCm;
        ...
        const float CoreHalfDepth = FMath::Max(HalfThickness - (CoreCenterGapCm * 0.5f), 1.0f);
        const float OffsetDist = (Run.WallThickness * 0.25f) + (CoreCenterGapCm * 0.25f); // Avoid coplanar internal faces between left/right core halves.
        ...
        BuildWallBox(
            ...
            false, // Wall core is structural only. Material editing should target WallSurface/WallExterior.
            Run.RunId,
            TEXT("WallCore_Left")
        );
